...
class Toggle extends React.Component {
  ...
  static Consumer = props => <ToggleContext.Consumer {...props}>{state => Toggle.getUI(props.children, state)}</ToggleContext.Consumer>;
  ...
  static On = ({ children }) => <Toggle.Consumer>{({ on }) => (on ? children : null)}</Toggle.Consumer>;
  static Off = ({ children }) => <Toggle.Consumer>{({ on }) => (on ? null : children)}</Toggle.Consumer>;
  static Button = props => <Toggle.Consumer>{({ getTogglerProps }) => <Switch {...getTogglerProps(props)} />}</Toggle.Consumer>;
  ...
  render() {
    ...
    // 🦉 By spreading the `rest` after `value`, it allows people
...
function withToggle(Component) {
  function Wrapper(props, ref) {
    ...
    return <Toggle.Consumer>{toggleState => <Component {...props} toggle={toggleState} ref={ref} />}</Toggle.Consumer>;
  }
